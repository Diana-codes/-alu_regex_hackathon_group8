...
IP: 10.0.0.1
"""
def print_episode_titles(text):
  """
  Prints all episode titles from the text on separate lines.
  """
  pattern = r"(?<=Episode: )(.+ S\d{2}E\d{2}: .+)"
  matches = re.findall(pattern, text)
  for match in matches:
      print(match)
print("Tv episodes title are:")
print_episode_titles(text)
print("\n")
